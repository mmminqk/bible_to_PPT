...
def process_text(input_text):
    # 정규식 설명:
    # ^ : 문자열 시작
    # (.*?) : 첫 번째 그룹 (가장 짧게 일치하는 모든 문자, 예: 1. 창 1:1)
    # \s+ : 공백 한 칸 이상
    # '.+ : 작은따옴표(')로 시작하는 지점부터 끝까지 무시
    
    # 입력 패턴 예시: "1. 창 1:1 '태초에' 굵게"
    # 결과: "1. 창 1:1" 만 추출
    pattern = r"^(.*?)\s+'.*"
    ...
    if match:
        # 매칭된 첫 번째 그룹(앞부분)만 반환
        return match.group(1).strip()
    else:
        # 따옴표 형식이 없을 경우를 대비해 전체 텍스트 반환 또는 예외처리
...
def parse_multi_refs_line(text):
    ...
    grouped_refs = []
    for line in lines:
        parts = line.strip().split(' ', 1) # 구절 번호와 주소를 분리
        if len(parts) < 2:
            continue
        ref_text = parts[1] # 주소만 선택
